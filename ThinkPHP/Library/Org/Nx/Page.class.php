...
class Page{
    ...
    public function showHis() {
        ...
        //上一页
        $up_row  = $this->nowPage - 1;
        $up_page = $up_row > 0 ? '<i class="previous item">' . $this->config['prev'] . '</i>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<i class="next item">' . $this->config['next'] . '</i>' : '';
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            #if($this->nowPage>1)exit('np:'.$this->nowPage);

            if($page > 0 && $page != $this->nowPage){

                if($page <= $this->totalPages){
                    $link_page .= '<span class="item">' . $page . '</span>';
                }else{
                    break;
                }
            }else{
                if($page > 0 && $this->totalPages != 1){
                    $link_page .= '<span class="on item">' . $page . '</span>';
                }
            }
        }
    ...
    public function showes() {
        ...
        //上一页
        $up_row  = $this->nowPage - 1;
        $up_page = $up_row > 0 ? '<a href="' . $this->url($up_row) . '">' .'<i class="previous">'. $this->config['prev'] ."</i>". '</a>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<a class="next layui-laypage-next" href="' . $this->url($down_row) . '">' .'<span class="on">'. $this->config['next'] .'</span>'. '</a>' : '';
...
/*        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
        ...
        $page_str = str_replace(
            ...
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages),
            $this->config['theme']);
        return '<div class="paging mt20 mb20 ftc">'."{$page_str}".'</div>';
    }
}
